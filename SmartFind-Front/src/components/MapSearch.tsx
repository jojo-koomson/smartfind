...
const MapSearch: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://smartfullapp-server2.vercel.app/api/data');
  ...
  const checkConnectivity = async (lat: number, lng: number) => {
    const postResponse = await axios.post('https://smartfullapp-server2.vercel.app/checkConnectivity', {
      clientlatitude: lat,
      clientlongitude: lng,
    });
